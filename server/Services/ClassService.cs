...
public class ClassService : IClassService {
    ...
    public async Task<ServiceResult<List<ClassSimpleDto>>> GetAllClasses(int page) {
        ...
        (List<Class> classList, int classCount) = await _classRepository.GetAllClasses(page);

        int numberPages = (int)Math.Ceiling((decimal)classCount / 10);

        return ServiceResult<List<ClassSimpleDto>>.Success(classList.Select(c => new ClassSimpleDto {
            Id = c.Id,
            UserId = c.User.Id,
